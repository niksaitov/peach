...
U64 findMagicNumber(int squareIndex, int relevantBits, bool fBishop)
{
    ...
    /*
    Set a limit on the occupancy index.
    A piece that controls N squares can have 2^N different occupancy combinations,
    so the maximum occupancy index is 2^relevantBits.
    For example, a bishop on a bad square moight control 6 other squares,
    so there would be no need to check more than 2^6 = 64 occupancy combinations.
    */
